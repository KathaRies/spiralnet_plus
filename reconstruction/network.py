...
class SpiralEnblock(nn.Module):
    def __init__(self, in_channels, out_channels, indices, conv):
        super(SpiralEnblock, self).__init__()
        self.conv = conv(in_channels, out_channels, indices)
            ...
            if idx == 0:
                self.en_layers.append(
                    SpiralEnblock(in_channels, out_channels[idx],
                                  self.spiral_indices[idx], conv=conv))
            else:
                self.en_layers.append(
            ...
            if idx == 0:
                ...
                self.de_layers.append(
                    SpiralDeblock(out_channels[-idx], out_channels[-idx - 1],
                                  self.spiral_indices[-idx - 1], conv=conv))
        self.de_layers.append(
            conv(out_channels[0], 3, self.spiral_indices[0]))
